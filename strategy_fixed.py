...
class MAStrategy:
    def __init__(self):
        ...
        self.short_window = 8        # 短期均线 - 从5调整到8
        self.long_window = 21        # 长期均线 - 从15调整到21
        self.rsi_period = 14        # RSI周期
        self.rsi_buy = 30           # RSI买入阈值 - 从35调整到30
        self.rsi_sell = 75          # RSI卖出阈值 - 从70调整到75
        self.volume_ratio = 1.2     # 成交量放大倍数 - 从1.3调整到1.2
        self.stop_loss = 0.04       # 止损比例 - 从0.05调整到0.04
        self.take_profit = 0.15     # 止盈比例 - 从0.20调整到0.15
        self.trailing_stop = 0.08   # 追踪止损比例 - 新增
    ...
    def get_history_data(self):
        ...
        try:
            ...
            start_date = (datetime.now() - timedelta(days=60)).strftime('%Y-%m-%d')  # 增加历史数据获取天数
            
            # 获取K线数据
            rs = bs.query_history_k_data_plus(
                self.security,
                "date,code,open,high,low,close,volume,amount,turn,pctChg",  # 增加更多字段
    ...
    def calculate_signals(self, df):
        ...
        try:
            ...
            df['MA60'] = df['close'].rolling(window=60).mean()  # 新增60日均线
            
            # 计算成交量均线
            df['volume_ma5'] = df['volume'].rolling(window=5).mean()
            df['volume_ma10'] = df['volume'].rolling(window=10).mean()  # 新增10日成交量均线
    ...
    def check_signals(self, df):
        ...
        try:
            ...
            bb_bounce = current['close'] < current['BB_Lower'] * 1.05  # 接近布林带下轨
            ...
            bb_pullback = current['close'] > current['BB_Upper'] * 0.95  # 接近布林带上轨
        ...
        try:
            if action == 'buy' and self.cash > 0:
                ...
                if shares >= 100:
                    ...
                    self.highest_price = price  # 初始化最高价
